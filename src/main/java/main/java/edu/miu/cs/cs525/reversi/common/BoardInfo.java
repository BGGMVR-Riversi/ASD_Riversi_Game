...
public class BoardInfo {
	...
	Logger logger =  Logger.getLogger(BoardInfo.class.getName());
	...
	private void goInADir(AnimationMatrix am, int rS, int cS, int rD, int cD) {
		...
		int rN = rS + rD;
		int cN = cS + cD;
		if (rN < 0 || rN >= ROW_COUNT || cN < 0 || cN >= COL_COUNT || board[rN][cN] == turn || board[rN][cN] == EMPTY) {
			return;
		}
		am.set(rN, cN, a);
	...
	private boolean checkInADir(int rS, int cS, int rD, int cD) {
		int rN = rS + rD;
		int cN = cS + cD;
		if (rN < 0 || rN >= ROW_COUNT || cN < 0 || cN >= COL_COUNT || board[rN][cN] == EMPTY) {
			return false;
	...
	public boolean isValidMove(int r, int c) {
		int iD, jD, dCount = 0;
		if (turn == NO_GAME || turn == GAME_OVER || board[r][c] != EMPTY) {
			return false;
		}
		board[r][c] = turn;
		for (iD = -1; iD <= +1; iD++) {
	...
	public int countCornerStablePieces(int player, Location corner, int rMAx, int cMax) {
		...
		boolean e;
		Location dir = new Location((corner.row <= ROW_COUNT / 2 ? +1 : -1),
				(corner.column <= COL_COUNT / 2 ? +1 : -1));
